...
function Abort($msg,$aref = NULL) {
  ...
  echo("<h3>PmWiki can't process your request</h3>
    ...
    <p><a href='$ScriptUrl'>Return to this site's home page</a></p>");
  if ($aref) {
...
function FreeLink($link) {
  ...
  $name = str_replace(' ',$PageNameSpace,
    preg_replace('/\\b(\\w)/e',"strtoupper('$1')",$name));
...
function Lock($op) {
  global $WikiDir,$LockFile;
  static $lockfp,$curop;
  SDV($LockFile,"$WikiDir/.flock");
  if (!$lockfp) {
...
function WritePage($pagename,$page) {
  global $Now,$WikiDir,$PageFileFmt,$Version,$Newline,$LastModFile;
  Lock(2);
...
function PrintText($pagename,$text="") {
  ...
  foreach($lines as $x) {
    ...
    if (preg_match("/^(:+)[^:]*:/",$x,$match)) {
      ...
      $x=preg_replace("/^(-+)&lt;/","<div class='outdent'>",$x);
      EmitCode("indent",strlen($match[1]));
    ...
    if (preg_match("/\\S.*($lp(\\d+)$lp)\\s*\$/",$x,$match)) {
      ...
      if (strstr($lpv[$match[2]]," img>")) $x=$rt.str_replace($match[1],"",$x);
