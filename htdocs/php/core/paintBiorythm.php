...
function pageHeader()
{
    ...
    print("<html><head>");
    print("<title>Rudolf Egger - Biorythmus</title>");
    print("</head><body>");
}
...
if(!isset($_REQUEST['birthdate']))
{
    ...
    ?>
     <form method="post" action="paintBiorythm.php">
     Bitte geben Sie ihren Geburtstag ein:<br>
     <input type="text" name="birthdate"
            value="TT.MM.JJJJ"><input type="submit" value="OK!">
     </form>
    <?php
...
if(!checkDate($birthMonth, $birthDay, $birthYear))
{
    ...
    print("Das Datum '$birthDay/$birthMonth/$birthYear' ist kein gültiges Datum.");
...
imageString($image, 3, 10, 10,  "Geburtstag: $birthDay.$birthMonth.$birthYear",
            $colorCross);
imageString($image, 3, 10, 26,  "heute:    ".  date(  "d.m.Y"),  $colorCross);
imageString($image, 3, 10, $diagramHeight - 42,  "Physiologisch", $colorPhysical);
imageString($image, 3, 10, $diagramHeight - 58,  "Emotional", $colorEmotional);
imageString($image, 3, 10, $diagramHeight - 74,  "Intellektuell",
            $colorIntellectual);
